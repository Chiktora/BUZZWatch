...
def upload_data_to_server(data, server_url):
    ...
    try:
        ...
        if response.status_code != 200:
            log_error_to_file("SERVER_UPLOAD_ERROR", f"Unexpected server response: {response.status_code} - {response.text}")
            return False
        return True
    except requests.ConnectionError as e:
        log_error_to_file("SERVER_CONNECTION_ERROR", f"Failed to connect to server: {e}")
        return False
...
def collect_and_store_data():
    ...
    try:
        ...
        if any(value is not None for value in sensor_data.values()) or image_path is not None:
            print("Conditions met for data insertion:")
            print(f"Sensor Data Valid: {any(value is not None for value in sensor_data.values())}")
            print(f"Image Path Valid: {image_path is not None}")
            insert_sensor_data(
